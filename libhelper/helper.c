 ...
 * write to lcd fifo
 */
LIBHELPER_EXPORT int write_lcd_fifo(int fd, unsigned char lcd_type,
				...
				char *str)
{
	...
	if (lcd_type == LCD1602) {
		len = LCD1602_MAX_COL + 1;
	} else if (lcd_type == LCD2004) {
		len = LCD2004_MAX_COL + 1;
	} else {
	...
	return 0;
out:
	perror("can't write to lcd fifo");

	return -1;
}

/*
 * write to lcd fifo (use struct lcd_request)
 */
LIBHELPER_EXPORT int write_lcd_fifo_2(int fd, struct lcd_request req)
{
	int err = -1;
	size_t len = 0;

	if (req.lcd_type == LCD1602) {
		len = LCD1602_MAX_COL + 1;
	} else if (req.lcd_type == LCD2004) {
		len = LCD2004_MAX_COL + 1;
	} else {
		eprintf("LCD is not supported!");
		return -1;
	...
	err = write(fd, &req.line, 1);
	if (err < 0)
		goto out;

	err = write(fd, &req.curs_pos, 1);
	if (err < 0)
		goto out;

	err = write(fd, &req.str, len);
	if (err < 0)
		goto out;
